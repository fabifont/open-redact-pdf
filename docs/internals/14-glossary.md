...
### Form XObject
A self-contained reusable content unit stored as a **stream** with its own operators, resources, and coordinate space. Invoked with the `Do` operator. Text extraction and search recurse into Form XObjects (applying the Form's `Matrix` to the CTM and preferring the Form's `Resources` over the page's). Redaction is intersection-aware and copy-on-write: when the Form's `BBox × Matrix × CTM` does not overlap any target, the Form is left alone; when it does, the engine allocates a per-page copy of the Form, rewrites that copy's content stream to strip the targeted glyph bytes, and points the page's `Resources.XObject` entry at the copy. Pages that share the original Form remain unaffected. Paint operators and nested `Do` inside a redacted Form are passed through unchanged. See `06-text-system.md §1` and `09-redaction-pipeline.md §6`.
