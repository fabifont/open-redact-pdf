...
### Form XObject
A self-contained reusable content unit stored as a **stream** with its own operators, resources, and coordinate space. Invoked with the `Do` operator. Text extraction and search recurse into Form XObjects (applying the Form's `Matrix` to the CTM and preferring the Form's `Resources` over the page's). Redaction is intersection-aware and copy-on-write: when the Form's `BBox × Matrix × CTM` does not overlap any target, the Form is left alone; when it does, the engine allocates a per-page copy of the Form and rewrites that copy to strip targeted text glyphs, neutralize vector paint operators that fall under a target, and replace intersecting Image `Do` invocations with `n`. Nested Forms are handled recursively up to depth 8, with each outer Form's `Resources.XObject` repointed at the redacted inner copy. Pages that share the original Forms remain unaffected. See `06-text-system.md §1` and `09-redaction-pipeline.md §6`.
