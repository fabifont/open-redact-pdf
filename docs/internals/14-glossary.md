...
### Form XObject
A self-contained reusable content unit stored as a **stream** with its own operators, resources, and coordinate space. Invoked with the `Do` operator. Text extraction and search recurse into Form XObjects (applying the Form's `Matrix` to the CTM and preferring the Form's `Resources` over the page's). Redaction is intersection-aware: a Form is left alone when its `BBox × Matrix × CTM` does not overlap any redaction target, and returns an explicit `Unsupported` error only when it does (because rewriting the Form's content stream in place is not yet implemented). See `06-text-system.md §1` and `09-redaction-pipeline.md §6`.
