...
- **No xref streams (PDF 1.5+):** The parser only supports classic xref tables. Cross-reference streams introduced in PDF 1.5 (using compressed object streams and stream-based xref) are not parsed. Most modern PDFs generated by up-to-date tools use xref streams, so this is a significant coverage gap.
