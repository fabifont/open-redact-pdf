...
- **Text-oriented stream filters:** The engine decodes `FlateDecode`, `ASCII85Decode`, and `ASCIIHexDecode`, including chained filter pipelines (e.g. `[/ASCII85Decode /FlateDecode]`). `LZWDecode`, `DCTDecode` (JPEG), `JBIG2Decode`, and `JPXDecode` are still rejected with an explicit error.
...
4. **Additional stream filters** — `LZWDecode` is the most commonly encountered unsupported filter after the FlateDecode / ASCII85Decode / ASCIIHexDecode set.
