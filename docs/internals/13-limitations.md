...
- **Simple fonts (Type1/TrueType) — ASCII only:** Glyph-to-Unicode mapping uses a direct byte-to-char cast, giving correct results only for ASCII. There is no `ToUnicode` CMap parsing and no `Encoding` dictionary support. Any byte value outside the printable ASCII range produces a U+FFFD replacement character.
