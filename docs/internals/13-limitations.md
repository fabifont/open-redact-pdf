...
- **Partial Image XObject rewriting (raw / `FlateDecode` / `DCTDecode`, 8-bpc DeviceGray/RGB/CMYK):** When a redaction target overlaps only part of an Image XObject, the underlying raster stream is rewritten in place via copy-on-write so multi-page-shared images are unaffected. The masked pixel rectangle is computed by inverse-transforming the target into the image's unit square and converting to integer pixel bounds; this is conservative for rotated CTMs (axis-aligned bounding box in image space). Images using `Indexed`, `ICCBased`, `JBIG2Decode`, `JPXDecode`, `CCITTFaxDecode`, or non-8-bpc colour storage fall back to whole-invocation drop. JPEG re-encode quality is fixed at 85 — the unredacted region is slightly degraded by the round-trip. `/SMask` (alpha) channels are left untouched: only the colour channels are masked.
